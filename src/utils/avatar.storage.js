...
var UPLOAD_PATH = path.resolve(__dirname, '..', process.env.AVATAR_STORAGE || 'uploads/avatars');
...
function AvatarStorage(opts) {
    ...
    var baseUrl = process.env.AVATAR_BASE_URL || '/uploads/avatars';
    ...
    this.uploadBaseUrl = this.options.responsive ? path.join('/uploads/avatars', 'responsive') : '/uploads/avatars';
...
AvatarStorage.prototype._handleFile = function(req, file, cb) {
    ...
    AvatarStorage.prototype._removeFile = function(req, file, cb) {
        ...
        if (this.options.responsive) {
            ...
            var matches = pathsplit.pop().match(/^(.+?)_.+?\.(.+)$/i);
        ...
        // Call the callback indicating successful removal
        cb(null);
    };
