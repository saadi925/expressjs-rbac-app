/*
  ...
  - Changed the type of `caseId` on the `CaseRequest` table. No cast exists, the column would be dropped and recreated, which cannot be done if there is data, since the column is required.
...
-- DropForeignKey
ALTER TABLE "CaseRequest" DROP CONSTRAINT "CaseRequest_caseId_fkey";

-- AlterTable
ALTER TABLE "Attachment" DROP COLUMN "caseId",
...
-- AlterTable
ALTER TABLE "CaseDetail" DROP COLUMN "caseId",
ADD COLUMN     "caseId" BIGINT NOT NULL;

-- AlterTable
ALTER TABLE "CaseRequest" DROP COLUMN "caseId",
ADD COLUMN     "caseId" BIGINT NOT NULL;
